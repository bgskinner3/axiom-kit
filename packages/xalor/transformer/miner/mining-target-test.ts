...
type TMiningTargetExtractor = {
  registerXalor: TMiningTargetRegistry;
  generateXalor: 
};
...
function resolveGenerateXalorCall(node: Node, checker: ts.TypeChecker) {
  if (!ts.isCallExpression(node)) return null;
  const typeArgs = node.typeArguments ?? [];
...
// function resolveRegisterXalorCall(node: Node, checker: ts.TypeChecker) {
//   if (!ts.isCallExpression(node)) return null;
