...
import { hydrateIntellisenseBridge } from '../../transformer/emitters/intellisense-bridge';
...
describe('Ambient Emitter (The Scribe)', () => {
  ...
  /**
   * 🏗️ Helper to create a valid payload for tests
   */
  const createMockPayload = (
    ...
    shape: { kind: 'primitive', type: 'unknown' }, // Minimal shape for emitter
    version: '1.0.0',
    ...overrides,
  });

  it('should generate a valid .d.ts file with Triple-KV mapping', () => {
    registry.set('USER', createMockPayload());

    hydrateIntellisenseBridge(rootDir, registry);

    const [, content] = (fs.writeFileSync as jest.Mock).mock.calls[0];

    // 💎 FIX: Use a Regex to match the import structure regardless of the relative depth
    // This matches: import('./.../src/models').IUser;
    expect(content).toMatch(/'USER': import\('\.\/.*src\/models'\)\.IUser;/);

    // 3. Verify Registry Layer (Structures)
    expect(content).toContain("'USER': { id: number; name: string };");
    // Check the comment exists and contains the filename
    expect(content).toMatch(/\/\*\* 📍 Origin: .*src\/models \*\//);
  });

  it('should handle anonymous/fragment types without importing symbols', () => {
    registry.set(
      'USER$d1',
      createMockPayload({
        key: 'USER$d1',
    ...
    expect(content).toContain("'USER$d1': { id: number; name: string };");

    // 🛡️ Ensure it did NOT create an import line for 'unknown'
    expect(content).not.toContain('import(');
    // Ensure the fallback line hint is present
    expect(content).toContain('// 📍 Line in 10');
  });

  it('should create the target directory recursively if it does not exist', () => {
  ...
  it('should handle cross-platform separators by normalizing to forward slashes', () => {
    // Use a relative-looking path to avoid drive-letter logic issues in POSIX test runners
    registry.set(
      'WIN_KEY',
      createMockPayload({
        filePath: 'C:/Users/Project/model.ts', // Use normalized slash for the mock input
        key: 'WIN_KEY',
      }),
    );
    ...
    // 💎 Verify normalization: The emitter should have converted any backslashes
    // (if any existed) and kept forward slashes for the TS import.
    expect(content).toContain('WIN_KEY');
    expect(content).toMatch(/import\('\.\/.*Users\/Project\/model'\)/);
  });
});

// // __tests__/transformer/emitter.test.ts
// import * as fs from 'fs';
// import { hydrateIntellisenseBridge } from '../../transformer/emitters';
