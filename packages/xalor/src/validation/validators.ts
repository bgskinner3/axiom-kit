...
export function validateArray(
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    if (!solidifyShape(data[i], shape.items, ctx)) {
...
export function validateObject(
  ...
  for (const [key, metadata] of propertyEntries) {
    ...
    // 2. Recurse
    if (!solidifyShape(value, metadata.shape, ctx)) {
      ctx.path = originalPath;
      return false;
    }
  }
...
export function validateUnion(
  ...
  for (const subShape of shape.values) {
    if (solidifyShape(data, subShape, ctx)) {
...
export function validateReference(
  ...
  return solidifyShape(data, metadata.shape, ctx);
}
/**
 * 💎 Internal Utility
